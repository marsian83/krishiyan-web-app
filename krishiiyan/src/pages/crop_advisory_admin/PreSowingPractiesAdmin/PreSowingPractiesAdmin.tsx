...
const ProductionReqContent = () => {
  ...
  const handlePreSowingSubmit = async () => {
    try{
      setLoading(true);
      if(!crop || !Land_Preparation || !Seed_treatment || !seed_doasage || !Soil_conditions || !Pre_Sowing_Operations){
      ...
      const res = await fetch(process.env.REACT_APP_BACKEND_URL + 'crop/role-admin/preSowing',{
        method : 'POST',
        headers : {
